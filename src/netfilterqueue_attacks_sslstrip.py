...
class NetfilterqueueAttacks:
    ...
    def trap_packets(self):
        ...
        subprocess.call(f"iptables -I INPUT -j NFQUEUE --queue-num {self.queue_num}", shell=True)
        subprocess.call(f"iptables -I OUTPUT -j NFQUEUE --queue-num {self.queue_num}", shell=True)
